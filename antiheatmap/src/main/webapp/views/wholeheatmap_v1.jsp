...
<script type="text/javascript">
    var BEGIN_X = (document.body.clientWidth - 1190)/2 + 23;
    var BEGIN_Y = (document.body.clientWidth - 1190)/2 + 23;

        var LIMIT_NUM = 20000;
        if( typeof(dispid) != "undefined" 
            || typeof(entid) != "undefined" 
            || typeof(clktime) != "undefined"
        ){
            drawSpmCoor(query_date,spm, dispid, entid, clktime);
        }
        else{
            var spm_click_num;
            $.ajax({
                type: "POST",
                data: "flag=1&query_date="+query_date+"&spm="+spm,
                dataType: "json",
                async: false,
                url: "./pdo_query.php",
                success: function(data){
                    spm_click_num = parseInt(data[0].click_num);
                }
            });
        
            for (var i = 0; i < spm_click_num/LIMIT_NUM; i++)
            {
                drawSpmHeatMap(query_date, 
                               spm,
                               LIMIT_NUM*i,
                               LIMIT_NUM
                );
        ...
        function drawSpmHeatMap(query_date, spm, start_query, query_num) {
            $.ajax({
                type: "POST",
                data: "flag=2&query_date="+query_date+"&spm="+spm+
                      "&start_query="+start_query+"&query_num="+query_num,
                dataType: "json",
                async: true,
                url: "./pdo_query.php",
                success: function(php_return_data) {
                    //打印php返回的json数据结果
                    console.log(php_return_data);
                    // 渲染热力图
                    draw(php_return_data);
                }
            });
        }

        function drawSpmCoor(query_date, spm, dispid, entid, clktime) {
            $.ajax({
                type: "POST",
                data: "flag=3&query_date="+query_date+"&spm="+spm+
                      "&dispid="+dispid+"&entid="+entid+"&clktime="+clktime,
                dataType: "json",
                async: true,
                url: "./pdo_query.php",
                success: function(php_return_data) {
        ...
        function draw(data) {
            var cvs_number = ctx = undefined;
            for (var i = 0; i < data.length; i++) {
                var ent_abs_x = Number(data[i]["entity_abs_x"]);
                var ent_abs_y = Number(data[i]["entity_abs_y"]);
                var container_y = Number(data[i]["container_y"]);
                var entity_y = Number(data[i]["entity_y"]);
                var fa_flag = Number(data[i]["fa_flag"]);
                var container_dis = Number(data[i]["container_dis"]);

                if (fa_flag == 191){
                    cvs_number = Math.round(container_dis/191)+1;
                    ctx = $('#191_' + cvs_number + 'th').get(0).getContext('2d');
                }
                else if (fa_flag == 217){
                    if (container_dis == 0){
                        ctx = $('#217_1th').get(0).getContext('2d');
                    }
                    else {
                        cvs_number = Math.round((container_dis-217)/191)+1;
                        ctx = $('#191_' + cvs_number + 'th').get(0).getContext('2d');
                    }
                }

                drawCoordinate(ctx, ent_abs_x, ent_abs_y);
